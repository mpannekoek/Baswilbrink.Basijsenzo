...
function sanitizeFieldName(fieldName: string): "primary" | "secondary" {
  return fieldName === "featuredTaste.imageSecondarySrc" ? "secondary" : "primary";
}

function resolveFeaturedTasteUploadDirectory(root = process.cwd()): string {
...
export async function storeFeaturedTasteImageUpload({
  fieldName,
  file,
  rootDirectory,
}: {
  ...
  const validationError = validateFeaturedTasteImageUpload(file);
